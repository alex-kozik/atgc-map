...
#     '*.z_nr_scores.loc' file has 'A' and 'B' scores only.                      #
#     Everything else is replaced by '-'                                         #
#                                                                                #
#     To work with unmodified non-redundant original scores use                  #
#     '*.w_nr_scores.loc' file                                                   #
#                                                                                #
...
def Read_Data_File(in_name, out_name, rec_cut, bit_cut, dat_cut, frame_file_name):
	...
	out_file3w_1 = open(out_name + '.w_nr_scores.loc', "wb")
	out_file3w_2 = open(out_name + '.w_scores_dupl', "wb")
	if map_construction != "NR_SET":
	...
	w_nr_scores_array = {}	; # ARRAY COMPLETE NON-REDUNDANT MARKER SCORES I
	w_nr_markers_array = {}	; # ARRAY COMPLETE NON-REDUNDANT MARKER SCORES II
	w_all_scores_array = {}	; # ARRAY COMPLETE WITH ALL MARKER SCORES
			...
			try:
				...
				while q < init_len:
					...
					if data_point == "A" or data_point == "B" or data_point == "C" \
						...
						# COMPLETE SCORES STRING #
						w_scores_string.append(data_point)
				...
				w_all_scores_array[id] = w_scores_string
				n = n + 1
			l = l + 1
...
##### COMPLETE W NR SET #####
	...
	w_nr_marker_list = []
	w_nr_good_list   = []
	w_really_bad     = []	; # ALL BAD MARKERS
	w_really_bad_allele = []	; # BAD WITH ALLELE DISTORTION
	w_really_bad_data   = []	; # BAD WITH LOSS OF DATA
	r_count = 0
	w_dupl_markers_list = {}
	w_good_string = {}
	w_ratio_AB = {}
	...
	for item in id_list:
		### MARKER SCORE DUPLICATION ###
		w_score_string = "\t".join(w_all_scores_array[item])
		w_text_string  = "".join(w_all_scores_array[item])
		count_nr_A[item] = w_text_string.count("A")
		count_nr_B[item] = w_text_string.count("B")
		count_nr_H[item] = w_text_string.count("H")
		count_nr_X[item] = w_text_string.count("-")
		w_good_string[item] = "BAD"
		w_ratio_AB[item] = -1
		w_bias_AB[item] = "XXX"
		if count_nr_A[item] == 0:
			w_good_string[item] = "BAD"
		...
		if count_nr_A[item] > 0 and count_nr_B[item] > 0:
			if count_nr_A[item] == count_nr_B[item]:
				w_ratio_AB[item] = count_nr_A[item]*1.0/count_nr_B[item]
				w_bias_AB[item] = "A=B"
			if count_nr_A[item] > count_nr_B[item]:
				w_ratio_AB[item] = count_nr_B[item]*1.0/count_nr_A[item]
				w_bias_AB[item] = "A>B"
			if count_nr_A[item] < count_nr_B[item]:
				w_ratio_AB[item] = count_nr_A[item]*1.0/count_nr_B[item]
				w_bias_AB[item] = "A<B"
		### README_005 ###
		if w_ratio_AB[item] >= allele_dist and count_nr_X[item] <= abs_loss:
			w_good_string[item] = "GOOD"
		if w_good_string[item] == "BAD":
			w_really_bad.append(item)
			if w_ratio_AB[item] < allele_dist:
				w_really_bad_allele.append(item)
			if count_nr_X[item] > abs_loss:
				w_really_bad_data.append(item)
		if w_score_string in w_nr_scores_list:
			print " *W* DATA DUPLICATION FOR MARKER:  " + item
			# time.sleep(1)
		### 
		if w_score_string not in w_nr_scores_list:
			w_nr_scores_list.append(w_score_string)
			w_nr_marker_list.append(item)
			if w_good_string[item] == "GOOD":
				###
				w_nr_good_list.append(item)
				out_file3w_1.write(item + '\t' + w_score_string + '\n')
			w_nr_scores_array[item] = w_all_scores_array[item]
			w_nr_markers_array[w_score_string] = item

	for item_x in id_list:
		# dupl_markers_list[item_x] = []
		for item_y in id_list:
			if w_all_scores_array[item_x] == w_all_scores_array[item_y] and item_x != item_y:
				# print "IDENTICAL DATA FOR PAIR: " + item_x + "  " + item_y
				working_id = "++++++"
				# if item_x in nr_marker_list:
				#	working_id = item_x
				# if item_y in nr_marker_list:
				#	working_id = item_y
				w_score_string = "\t".join(w_all_scores_array[item_x])
				try:
					working_id = w_nr_markers_array[w_score_string]
					# dupl_markers_list[item_y] = working_id
					w_dupl_markers_list[item_x] = working_id
				except:
					working_id = "------"
				### README_004 ###
				print " *W* IDENTICAL DATA FOR PAIR: " + item_x + "  " + item_y + "    WORKING ID: " + working_id
				out_file3w_2.write(item_x + '\t' + "==" + '\t' + item_y + '\t' + "***" + '\t' + working_id + '\n')
